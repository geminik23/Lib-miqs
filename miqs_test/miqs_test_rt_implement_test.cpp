...
void miqs_TEST_OBJ_NAME::process(sample_t* in, size_t nchi, sample_t * out, size_t ncho, size_t l)
{
	miqs::array<size_t> temp(512);

	(void)in; (void)nchi; (void)ncho; (void)l;
	miqs::array_accessor<sample_t> i1{ in , l };
	miqs::array_accessor<sample_t> i2{ in+l , l };

	miqs::array_accessor<sample_t> o1{ out , l };
	miqs::array_accessor<sample_t> o2{ out + l , l };

	// check
	if (m_samples.size() - (l * 2) < m_cur_position)
	{
	...
	////miqs::normalize(miqs::ptr_begin(o1), miqs::ptr_end(o1), 0.89);
